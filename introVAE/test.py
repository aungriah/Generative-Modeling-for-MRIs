...
with tf.Session() as session:
    for i in range(2):
        ...
        for i in range(6):
            zend = z_x[i+1]
            zs = interpolate(i+1,z_x[0],xend)
            interp = session.run(xr,feed_dict={z:zs})
        ...
        #if (global_iters % args_frequency) == 0:
        ...
        print('Epoch: {}/{}, iteration: {}/{}'.format(epoch+1, args_nb_epoch, 1+1, 1))
        ...
        saved = saver.save(session, "model", global_step=global_iters)
        print('Saved model to ' + saved)
        n_x = 5
        n_y = args_batch_size // n_x
        print('Save original images.')
        utils.plot_images(np.transpose(x, (0, 2, 3, 1)), n_x, n_y, "original")
        print('Save generated images.')
        utils.plot_images(np.transpose(x_p, (0, 2, 3, 1)), n_x, n_y,"sampled")
        print('Save reconstructed images.')
        utils.plot_images(np.transpose(x_r, (0, 2, 3, 1)), n_x, n_y, "reconstructed")
